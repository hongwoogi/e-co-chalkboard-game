...
(function registerColorStack() {
  ...
  function init(container, options) {
    ...
    `;
    hud.innerHTML = `
      <span id="cs-score-${playerIndex}" style="font-family:var(--font-display);font-size:1.1rem;font-weight:bold;color:${playerColor};">0점</span>
      <span id="cs-count-${playerIndex}" style="font-family:var(--font-body);font-size:0.78rem;color:#888;"></span>
      <span id="cs-timer-${playerIndex}" style="font-family:var(--font-display);font-size:1.1rem;font-weight:bold;color:#3d2b1f;">${gameDuration}s</span>
    `;
    ...
    function renderStack() {
      ...
      stack.forEach((color, i) => {
        ...
        block.style.cssText = `
          ...
          ${isBottom ? 'animation:csBottomIn 0.25s ease-out both;' : ''}
        `;
        block.innerHTML = `
        ...
        if (isBottom) {
          ...
          arrow.style.cssText = `
            ...
            animation:csArrow 0.55s ease-in-out infinite alternate;
          `;
          arrow.textContent = '◀';
      ...
      const countEl = container.querySelector(`#cs-count-${playerIndex}`);
    ...
    function showBonus() {
      ...
      flash.style.cssText = `
        ...
        animation:csBonus 0.8s ease-out forwards;
      `;
    ...
    function updateHUD() {
      const scoreEl = container.querySelector(`#cs-score-${playerIndex}`);
      const timerEl = container.querySelector(`#cs-timer-${playerIndex}`);
    ...
    function endGame() {
      ...
      const key  = `color-stack-score-p${playerIndex}`;
  ...
  window.GameModules['color-stack'] = { init };
